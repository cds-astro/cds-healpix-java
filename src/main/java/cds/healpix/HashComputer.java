 ...
 * to reuse objects inside an instance. It means that object implementing {@link HashComputer}
 * are probably NOT THREAD-SAFE. So you MUST have one such implementation object per thread.
 * 
 * @author F.-X. Pineau
 *
 */
public interface HashComputer extends HierarchyItem {
  ...
  /**
   * Returns the depth of the hash computed by {@link #hash(double, double)} method.
   * @return the depth of the hash computed by {@link #hash(double, double)} method.
   */
  @Override
  int depth();
   ...
   */
  long hash(final double lonRad, final double latRad);
